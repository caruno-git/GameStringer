...
CREATE INDEX IF NOT EXISTS idx_packs_game ON translation_packs(game_name);
CREATE INDEX IF NOT EXISTS idx_packs_language ON translation_packs(target_language);
...
CREATE INDEX IF NOT EXISTS idx_reviews_pack ON pack_reviews(pack_id);
...
CREATE INDEX IF NOT EXISTS idx_comments_pack ON pack_comments(pack_id);
...
DROP POLICY IF EXISTS "Authors read own packs" ON translation_packs;
...
DROP POLICY IF EXISTS "Users create packs" ON translation_packs;
CREATE POLICY "Users create packs" ON translation_packs FOR INSERT WITH CHECK (author_id = auth.uid());
DROP POLICY IF EXISTS "Users update own packs" ON translation_packs;
CREATE POLICY "Users update own packs" ON translation_packs FOR UPDATE USING (author_id = auth.uid());
DROP POLICY IF EXISTS "Users delete own packs" ON translation_packs;
...
CREATE INDEX IF NOT EXISTS idx_shared_tm_hash ON shared_tm(source_hash);
CREATE INDEX IF NOT EXISTS idx_shared_tm_game ON shared_tm(game_app_id);
CREATE INDEX IF NOT EXISTS idx_shared_tm_game_name ON shared_tm(game_name);
CREATE INDEX IF NOT EXISTS idx_shared_tm_langs ON shared_tm(source_language, target_language);
CREATE INDEX IF NOT EXISTS idx_shared_tm_confidence ON shared_tm(confidence DESC);
CREATE INDEX IF NOT EXISTS idx_shared_tm_contributors ON shared_tm(contributor_count DESC);

-- RLS
ALTER TABLE shared_tm ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Public read shared TM" ON shared_tm;
